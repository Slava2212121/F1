...
import { Calendar as CalendarIcon, MapPin, Flag, ChevronRight, Trophy, AlertTriangle, Heart } from 'lucide-react';
...
export const F1_2026_CALENDAR: Race[] = [
  ...
  { id: 21, round: 19, country: 'Бразилия', countryCode: 'BR', name: 'Гран-при Сан-Паулу', circuit: 'Сан-Паулу', date: '9 Ноября', fullDate: '2026-11-09', status: 'UPCOMING', flag: '🇧🇷' },
  { id: 22, round: 20, country: 'США', countryCode: 'US', name: 'Гран-при Лас-Вегаса', circuit: 'Лас-Вегас', date: '22 Ноября', fullDate: '2026-11-22', status: 'UPCOMING', flag: '🇺🇸' },
  { id: 23, round: 21, country: 'Катар', countryCode: 'QA', name: 'Гран-при Катара', circuit: 'Лусаил', date: '29 Ноября', fullDate: '2026-11-29', status: 'UPCOMING', flag: '🇶🇦' },
  { id: 24, round: 22, country: 'ОАЭ', countryCode: 'AE', name: 'Гран-при Абу-Даби', circuit: 'Яс-Марина', date: '6 Декабря', fullDate: '2026-12-06', status: 'UPCOMING', flag: '🇦🇪', note: 'Финал сезона' }
];
...
export const CalendarView: React.FC<CalendarViewProps> = ({ firebaseData = [] }) => {
  ...
  useEffect(() => {
    if (firebaseData && firebaseData.length > 0) {
      const merged = F1_2026_CALENDAR.map(race => {
        // Find by name or id
        const fbRace = firebaseData.find(r => r.name === race.name || r.id === race.id?.toString());
        return fbRace ? { ...race, ...fbRace } : race;
      });
      setRaces(merged);
    }
  }, [firebaseData]);
  ...
  React.useEffect(() => {
    const fetchWeather = async () => {
      const updatedRaces = await Promise.all(races.map(async (race) => {
        if (race.status === 'NEXT') {
          try {
            const response = await fetch(`https://wttr.in/${encodeURIComponent(race.circuit)}?format=j1&lang=ru`);
            const data = await response.json();
            const weather = data.current_condition?.[0]?.lang_ru?.[0]?.value || data.current_condition?.[0]?.weatherDesc?.[0]?.value || 'N/A';
            const temp = data.current_condition?.[0]?.temp_C ? `Воздух: ${data.current_condition[0].temp_C}°C` : '';
            return { ...race, weather, temp };
          } catch (error) {
            console.error(`Error fetching weather for ${race.circuit}:`, error);
  ...
  const groupedRaces = React.useMemo(() => {
    return races.reduce((acc, race) => {
      if (!race.fullDate) return acc;
      const date = new Date(race.fullDate);
      ...
      acc[capitalizedMonth].push(race);
      return acc;
  ...
  return (
    <div className="pt-6 pb-24 px-4 space-y-4 max-w-4xl mx-auto bg-f1-black min-h-screen text-white">
      ...
      <div className="space-y-8">
        {Object.entries(groupedRaces).map(([month, monthRaces], monthIndex) => (
          <div key={month} className="space-y-4">
            <h2 className="text-xl font-display font-bold text-gray-300 border-b border-white/10 pb-2 sticky top-16 bg-f1-black/90 backdrop-blur-md z-20 px-2">
              {month}
            </h2>
              ...
              {monthRaces.map((race, index) => (
                <motion.div 
                  ...
                  className={`rounded-2xl overflow-hidden transition-all duration-300 bg-f1-dark border border-f1-carbon ${
                    race.results && race.results.length > 0 && race.status === 'COMPLETED' ? 'cursor-pointer hover:border-f1-gold/50' : ''
                  } ${
                    race.status === 'NEXT' 
                ...
                >
                  ...
                  <div className="p-3 sm:p-4 flex items-center gap-2 sm:gap-4">
                    <button onClick={(e) => toggleFavorite(e, race.id)} className="mr-2">
                      <Heart size={20} className={favorites.includes(race.id) ? 'fill-f1-gold text-f1-gold' : 'text-f1-carbon'} />
                    </button>
                        ...
                        <p className="text-[10px] sm:text-xs text-blue-400 mt-1 flex items-center gap-1">
                          <span className="flex-shrink-0">
                            {race.weather.toLowerCase().includes('sunny') || race.weather.includes('Ясно') ? '☀️' : 
                             race.weather.toLowerCase().includes('cloudy') || race.weather.includes('Облачно') || race.weather.includes('Пасмурно') ? '☁️' : 
                             race.weather.toLowerCase().includes('rain') || race.weather.includes('Дождь') ? '🌧️' : '🌤️'}
                          </span>
                          <span className="truncate">
                            {race.temp ? `${race.temp.replace('Воздух: ', '')}, ` : ''}{
                    ...
                    <div className="flex flex-col items-end flex-shrink-0 ml-1">
                      ...
                      {race.results && race.results.length > 0 && race.status === 'COMPLETED' && (
